...
export const signUp = asyncHandler(async (req, res) => {
    ...
    if (!firstName || !lastName || !email || !password || !role || !contact) {
...
export const login = asyncHandler(async (req, res) => {
    ...
    if (isPasswordMatched) {
        if (user.status === "BLOCKED") {
            throw new CustomError("Your account has been blocked, Please Contact Admin", 400);
        }

        if (user.status === "UNVERIFIED") {
            throw new CustomError("Please verify your email", 400);
        }
...
export const resetPassword = asyncHandler(async (req, res) => {
    ...
    const storedToken = Token.findOne({ user: user._id });
    const isTokenValid = await storedToken.verifyToken(resetToken);
